...
import { fetchInstrumentsIndex, fetchInstrumentIndex } from '../trades-index-loader';
...
export const InstrumentsView = ({ instrumentType }) => {
    ...
    useEffect(() => {
        setLoading(true);
        fetchInstrumentsIndex(instrumentType)
            .then((instruments) => {
                const arr = Object.entries(instruments).map(([key, val]) => ({
                    id: key,
                    ticker: val.ticker,
    ...
    const allSelection = useMemo(() => model.length > 0 && model.every((e) => selectedInstruments[keyFn(e)]), [model, selectedInstruments]);
                ...
                body={model.map((instrument) => [
                    ...
                    <Tooltip>
                        <Tooltip.Trigger asChild>
                            <Button appearance="flat-grayscale-on-dark" onClick={() => setSelectedInstrument(instrument)}>
