...
:::info
Добавили новый метод загрузки исторических данных — рекомендуем перейти на его использование. [Подробнее](/invest/services/history-md/head-history-md).
:::

Для загрузки истории котировок можно воспользоваться интерфейсом, специальным методом или подготовленными 
[скриптом и справочником FIGI](https://github.com/RussianInvestments/investAPI/tree/main/src/marketdata).
...
Чтобы получить исторические данные, нужно ввести [токен доступа](/invest/intro/intro/token).
...
Чтобы получить историю котировок за весь доступный период, можно воспользоваться специальным [скриптом](https://github.com/RussianInvestments/investAPI/blob/main/src/marketdata/download_md.sh) и 
[справочником FIGI](https://github.com/RussianInvestments/investAPI/blob/main/src/marketdata/figi.txt).
...
Перед запуском скрипта `download_md.sh` нужно добавить [токен доступа](/invest/intro/intro/token) в параметр `token`.
...
|token  | [Токен доступа](/invest/intro/intro/token)                                                |


### Результат работы скрипта

Когда скрипт успешно исполнен, в директорию, откуда запущен скрипт, будут сохранены ZIP-архивы. Они разбиваются по 
инструментам и годам исторических рыночных данных.
